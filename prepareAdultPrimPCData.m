...
CaseTableAdult = caseTableArray{1};

%% STEP2: Loop through cols & rows of CaseTable Table, add questions as cols of Feature Table and ans as values
FeaturesTableAdult = CaseTableAdult(:,[1 3 5:18]);
for c = 1:size(CaseTableAdult,2)
    currentColName = CaseTableAdult.Properties.VariableNames{c};
    if strfind(currentColName,'questxt')
        for r = 1:size(CaseTableAdult,1)
           %Add the question as a feature
           if isempty(CaseTableAdult{r,c}{1})
               continue
           elseif ~any(ismember(FeaturesTableAdult.Properties.VariableNames, lower(regexprep(CaseTableAdult{r,c},'[\W\d]',''))))
               tempCol = cell(size(FeaturesTableAdult,1),1);
               tempCol(:,1) = {'Unsure'};
               eval(['FeaturesTableAdult.' char(lower(regexprep(CaseTableAdult{r,c},'[\W\d]',''))) '=tempCol;'])
           end
           %Add the answer as a value & replace with 'Unsure' if an unsuretiy
           ansVal = CaseTableAdult{r,c+1}{1};
           if isempty(ansVal) || strcmp(ansVal,'Unsure')|| strcmp(ansVal,'#N/A') || strcmp(ansVal,'Not known') || strcmp(ansVal,'Not specific') || strcmp(ansVal,'Not assessed') || strcmp(ansVal,'Nil specific') || strcmp(ansVal,'Unsure/explore')
               eval(['FeaturesTableAdult.' char(lower(regexprep(CaseTableAdult{r,c},'[\W\d]',''))) '(' num2str(r) ')' '={''Unsure''};'])
           else
               eval(['FeaturesTableAdult.' char(lower(regexprep(CaseTableAdult{r,c},'[\W\d]',''))) '(' num2str(r) ')' '={ansVal};'])
           end
        end
    end
end
...
pcList = {'overall'};
FeaturesAnalysisAdult = cell2table(cell(size(FeaturesTableAdult,2),size(pcList,2)));
FeaturesAnalysisAdult.Properties.VariableNames = pcList;
FeaturesAnalysisAdult.Properties.RowNames = FeaturesTableAdult.Properties.VariableNames;
for c = 17:size(FeaturesTableAdult,2)
    %Set missing count variable =0
    pcMissing = 0;
    %Count missing values for given feature
    for r = 1:size(FeaturesTableAdult,1)
        if strcmp(FeaturesTableAdult{r,c}{1},'Unsure') 
            pcMissing = pcMissing+1;
        end         
    end
    % Add Cacluated Percentage for given feature to all pc structs
    pcOverall = size(FeaturesTableAdult,1);
    FeaturesAnalysisAdult.overall{c} = (pcOverall-pcMissing)/pcOverall;
end

%% STEP4: Create X & Y inputs for triageSVMLib
% Initialize X for given pc
XY_pc_Adult = FeaturesTableAdult(:,[2 15:16]);

%Add features with >5% data within respective pcs
for c = 17:size(FeaturesTableAdult,2)
    if FeaturesAnalysisAdult.overall{c} >= .05
        XY_pc_Adult = [XY_pc_Adult FeaturesTableAdult(:,c)];
    end
end

%% STEP4: Create X & Y inputs for triageBNTs
% Initialize X for given pc
XY_BNT = FeaturesTableAdult(:,2);

%Add features with >5% data within respective pcs
for c = 17:size(FeaturesTableAdult,2)
    if FeaturesAnalysisAdult.overall{c} >= .05
        XY_BNT = [XY_BNT FeaturesTableAdult(:,c)];
    end
end

% XY_BNT = FeaturesTable(:,[ 2 17 34 44 45 57 63 84 89 111 112 128 138 155 158 181 ]);

%% STEP5: Detect triageSVM performance on each X/Y
XY_pc_SFS = FeaturesTableAdult(:,[ 2 17 34 44 45 57 63 84 89 111 112 128 138 155 158 181 ]);
